...
func (o *OptionsInstallRun) Run(args []string) error {
	...
	if o.Mode == "docker" {
		...
		//harborDir := fmt.Sprintf("%s/%s", installDockerConfig.RootDir, installDockerConfig.ImageRepoDir)
		...
		//	return err
		//}
		//LogSuccess(fmt.Sprintf("create harbor certificates %s/%s success", harborDir, installDockerConfig.ImageRepo.CertsDir))
		//
		//// pull docker images
		...
		//	curl -k -X POST "https://admin:%s@%s/api/v2.0/projects" -H  "accept: application/json" -H  "Content-Type: application/json" -d '{"project_name": "quay", "public": true}'`, installDockerConfig.ImageRepo.Password, installDockerConfig.ImageRepo.DomainName, installDockerConfig.ImageRepo.Password, installDockerConfig.ImageRepo.DomainName, installDockerConfig.ImageRepo.Password, installDockerConfig.ImageRepo.DomainName, installDockerConfig.ImageRepo.Password, installDockerConfig.ImageRepo.DomainName)
		...
		//	targetImage := fmt.Sprintf("%s/%s", installDockerConfig.ImageRepo.DomainName, idi.Target)
		...
		//(kubectl delete pv project-data-pv || true)`, installDockerConfig.Kubernetes.Namespace, installDockerConfig.Kubernetes.Namespace)
